...
这个协议要解决的是当前 Agent 系统里的一个普遍痛点：一旦任务需要外部工具、私有基础设施、领域专用工作流，或由另一套运行时长期维护的能力，接入方式往往就会退化成零散 API、prompt 约定和项目私有胶水代码。这样不仅难以复用、难以验收，也很难在不同宿主 Agent 之间迁移。`Remote Subagent Protocol` 采用 transport-neutral、contract-first 的方式，把目录发现、任务授权、投递、签名结果和信任积累收敛成统一协议链路，使 Buyer Agent 调用远程能力时，面对的是稳定协议能力，而不是一次性集成脚本。
