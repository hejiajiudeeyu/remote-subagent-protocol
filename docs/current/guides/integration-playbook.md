...
10. 当前实现：Seller 在结果确定后额外向平台写入 `COMPLETED` 或 `FAILED` 事件，作为控制面观测，不替代结果回传。
11. Buyer 轮询服务端请求事件与结果，按 `request_id` 验签+验 schema（含错误结果包）。
12. Buyer/Seller 分别调用 `POST /v1/metrics/events` 上报最小指标。
...
- 同一事件流还可观察到 Seller 写入的 `COMPLETED/FAILED` 完成态；这些事件仅用于控制面观测与辅助排障，不直接决定 Buyer 本地终态。
...
- 状态迁移：
  - v0.1 不实现 `RUNNING/PROGRESS` 进度事件；Buyer 控制面通常观察到 `ACKED` 后等待 transport 结果，必要时可额外看到 `COMPLETED/FAILED` 观测事件
...
- 若该运行时要承载 Remote Subagent，则通过 `POST /v1/catalog/subagents` 提交 onboarding 草案；seller 与 subagent 都需管理员审批后才会进入公开目录。
- seller 本地 runtime 可先启动；但仅当 `seller.review_status=approved`、`subagent.review_status=approved` 且双方 `status=enabled` 时，才可被 Buyer 发现和远程调用。
...
12. 完成态观测：Seller 在结果包确定后调用 `POST /v1/requests/{request_id}/events` 写入 `COMPLETED` 或 `FAILED`。
...
- **公钥变更**：触发双 key 窗口（见 `../spec/architecture.md` 第 5.6 节），买家验签自动兼容。
...
- 能力声明模板：`docs/templates/subagents/{subagent_id}/`（详见 `../spec/architecture.md` §4.5）
