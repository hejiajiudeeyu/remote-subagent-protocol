...
3. `agent-registration-call-flow.md`：post-L0 的 remote subagent onboarding、审核、导入与 seller 激活。
...
- 文档分层与真相源判定统一以 `doc-truth-source-map.md` 和 `../l0/architecture.md` 第 1.4 节为准。
- 当前 ACK 回传模式为 Pull（`GET /v1/requests/{request_id}/events`）。
- 接口字段与返回结构以 `../l0/platform-api-v0.1.md` 为唯一规范源；图中若与 API 文档冲突，以 API 文档为准并需回补改图。
- Buyer 超时确认默认策略：`soft_timeout` 询问、`hard_timeout` 自动终态 `TIMED_OUT`；该语义不等价于远端进程 kill。
- L0 之后的 onboarding、搜索、外部 transport、多轮对话等演进内容统一收敛到 `../post-l0-evolution.md`。
